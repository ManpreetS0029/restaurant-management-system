  ...
  +
        ...
        <div class="col">
            ...
            <table class="table table-striped">
             ...
             <?php
            $getTime = $conn->prepare("select DATE_FORMAT(order_time, '%H:%i:%s') DATEONLY from order_table where order_table = ? and order_status = 'Completed' ");
            $getTime->execute(array($_GET["tbl_name"]));
            $time = $getTime->fetch();

            //print_r($time);

             $orderId = ltrim($_GET["tbl_name"],"Table");
             $query = $conn->prepare("select * from order_item_table where order_id = ? and time != '".$time[0]."' limit ". $page_first_result . ',' . $resultsPerPage);
             ...
             echo "<tr>";
                 ...
                 <form method='post' action='edit-order.php?page=".$pg."&id=".$order['order_id']."&item=".$order['product_name']."&tbl_name=".$_GET['tbl_name']."'>
